import type { ReturnRefund } from '../../data/returnsData'
...
export function ReturnsTable({ rows, startIndex }: ReturnsTableProps) {
  if (rows.length === 0) {
    return (
          ...
          <tbody>
            {rows.map((row, index) => (
              ...
              >
                ...
                <td className="px-3 py-3 whitespace-nowrap text-ink-muted">{formatDate(row.date)}</td>
                <td className="px-3 py-3 text-ink">{row.invoiceId}</td>
        ...
        {rows.map((row, index) => (
          <li key={row.id} className="p-4">
            ...
            <p className="mt-1 text-sm text-ink-muted">
              {row.customer} · Invoice {row.invoiceId}
            </p>
            <div className="mt-2 flex items-center justify-between gap-2">
              <p className="text-xs text-ink-muted">{formatDate(row.date)}</p>
              <p className="text-sm font-semibold text-ink">{formatCurrency(row.total)}</p>
            </div>
          </li>
        ))}
      </ul>
